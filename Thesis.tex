...
\begin{equation}
\hat{H} =  \frac{\hbar w_{ge}}{2} \hat{\sigma}_z +\hbar f \hat{\sigma}_x \cos \left(\omega t + \phi\right)
\label{eq:ham-qubit-2d-field}
\end{equation}
, здесь $f$ отвечает за амплитуду поля. 
\newline
...
%$$ i \hbar \hat{U}^{+} \frac{\partial}{\partial t}  \tilde{\psi} + i \hbar \frac{\partial \hat{U}^{+}}{\partial t}  \tilde{\psi} = \hbar{H} \hat{U}^{+} \tilde{\psi} $$
...
\begin{equation}
 ...
 \frac{i}{\hbar}\sbrkt{\hat \rho_s, \mathcal{\hat H}_s} + \sum_k \frac{\gamma_k}{2} \mathcal{D}\sbrkt{\mathcal{\hat O}_k}\hat \rho_s,
...
\begin{equation}
\frac{\partial}{\partial t}\hat \rho_s = 
 \frac{i}{\hbar}\sbrkt{\hat \rho_s, \mathcal{\hat H}_s} +  \frac{\gamma}{2} \mathcal{D}\sbrkt{\hat C}\hat \rho_s,
...
\iffalse
----------------TODO------------------
вставить картинки с релаксацией и дефазировкой на сфере Блоха
\fi

\subsection{Утечки на более высокие уровни}
Другой нежелательный (чаще всего) эффект, заключающийся в возбуждении более высоких уровней, не связан со взаимодействием с окружением кубита. Однако, он может проявиться при слишком большой амплитуде поля $f$ в (\ref{eq:ham-qubit-2d-field}). В пункте \ref{chap:transmon} было сказано, что величина ангармонизма задает минимально время операций, осуществляемых с помощью микроволновых импульсов. Поясним теперь суть этого утверждения. При $f > 0$ возможны переходы в области порядка $\omega \pm f$, значит, при $f \sim \alpha$ возможны переходы $\ket{1} \rightarrow \ket{2}$ с энергией $\omega + \alpha$. 
\texttt{abcd x = y abcd}
